  ...
  \item \label{qst:3}
    \begin{enumerate}
      \item \label{qst:3a}
        ...
        and partition $A$ around that element. Since $k_{\floor{m/2}}$ is the rank of $x$, all the
        elements with ranks less than $k_{\floor{m/2}}$ will be to the left of $x$, and all the
        elements with ranks greater than $k_{\floor{m/2}}$ will be to the right of $x$. But this
        problem is structurally similar to the initial problem, so we invoke two additional recursive
        calls---one on the left and one on the right---to conquer these two subproblems with recursion.
        We stop when the set of ranks $K$ becomes of size one, in which case we have the canonical
        selection problem which can be easily tackled with quick select. The pseudocode for this is
        given below.
          ...
          \begin{algorithm}[H]
            ...
            \begin{algorithmic}[1]
              ...
              \Else
                ...
                \State $A_L \gets \{a \in A \mid a \leq x\}$ \Comment{aggregate all entries of $A \leq x$}
                ...
                \State $K_L \gets \{k \in K \mid k \leq r\}$ \Comment{aggregate all entries of $K \leq r$}
      ...
      \item \label{qst:3c}
        ...
        \begin{itemize}[itemsep=0pt]
          ...
          \item
            {\itshape Inductive Step}: now consider when $|K| = m$. Steps $10$ and $11$ of
            \autoref{alg:multi-select} partitions $K$ into two (disjoint) sublists of nearly equal
            sizes. Let $K_L$ and $K_R$ be these lists. Clearly, $|K_L| < m$ and $|K_R| < m$. Thus
            applying the induction hypothesis, we are guaranteed that the two {\sc Multi-Quick-Select}
            calls in Steps $12$ and $13$ of \autoref{alg:multi-select} correctly report the $k$-th
            smallest number of $A_L$ (for every $k \in K_L$) and the $k$-th smallest number of $A_R$
            (for every $k \in K_R$). Therefore, a total of $|K_L| + |K_R| = |K| = m$ elements of $K$
            are correctly reported, which is exactly what we wanted to show! \hfill $\square$
        \end{itemize}
      ...
      \item \label{qst:3d}
        ...
        count the assignment calls of Steps $7$ and $8$ to the total running time. Additionally, per
        the problem definition, the entries of $K$ are all distinct integers ranging from $1$ to $n$
        so $|K| = m \leq n$. Thus constructing $K_L$ and $K_R$ in Steps $10$ and $11$ incurs at most
  ...
  \item \label{qst:4}
    \begin{enumerate}
      \item \label{qst:4a}
        ...
        scenarios at hand: (1) the sum is greater than $M$, in which case we recurse on the left;
        and (2) the sum is less or equal to $M$, in which case, we offset $M$ by that sum and recurse
        on the right (see \autoref{alg:maximal-element} for details).
      ...
      \item
        ...
        \begin{minipage}{\linewidth}
          \begin{algorithm}[H]
            \caption{{\sc Maximal-Element}$(A, M)$}\label{alg:maximal-element}
            \begin{algorithmic}[1]
