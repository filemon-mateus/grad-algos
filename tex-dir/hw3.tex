...
\begin{enumerate}[leftmargin={*}, font={\bf}, label={\arabic*.}, ref={\arabic*}]
  \item \label{qst:1}
    \begin{enumerate}
      \item \label{qst:1a}
        Let $R$ be the set of red terminal blocks and $B$ the set of blue terminal blocks.
        For each red terminal block $r_i \in R$, let $l_{r_i} \geq 0$ be the length of its
        wire. Analogously, for each blue terminal block $b_j \in B$, let $l_{b_j}$ be the
        length of its wire. Let $d_{ij}$ be the eucledian distance between red block $r_i$
        and blue block $b_j$, i.e., $d_{ij} = || r_i - b_j||$. Then the LP formulation will
        be given as follows:
        \begin{gather*}
  ...
  \item \label{qst:2}
    ...
    value at most $z$. This follows from the fact that $\min(c^Tx) \leq c^Tx \leq z$ and
    the existence of an optimum $\min(c^Tx) > z$ in $\mathcal{P}$ violates the second
    linear constraint of $\mathcal{G}$; thereby, making it infeasible.
