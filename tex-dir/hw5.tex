  ...
  \item \label{qst:1}
    \begin{enumerate}
      \item \label{qst:1a}
        ...
        there exists a multiset of items $1, \ldots, i$ which size exactly $j$. With a abuse of
        set notation we define:
        \[
        ...
        \begin{enumerate}[label={(\arabic*)}, itemsep={0pt}]
          \item
            ...
            \ldots, i$ that size exactly $j-a_i$. Here $f[i][j-a_i]$ indicates whether such multiset
            exists.
          \item
            Item $i$ does not occur in $S$. In this case, the solution maps to a multiset of the
            items $1, \ldots, i-1$ that size exactly $j$. The answer of whether such multiset exists
            is found in $f[i-1][j]$.
        \end{enumerate}
        ...
        them evaluate to \textsc{True}---note that this is achieved by taking the max across the
        intermediate answers to $f[n][M]$.
