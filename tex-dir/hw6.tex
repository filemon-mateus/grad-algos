  ...
  \item
    ...
    \begin{proof*}
      ...
      \begin{description}[itemsep={0pt}]
        ...
        \item[$\Longleftarrow$]
          ({\itshape Sufficient Condition}) --- If $e$ is not {\it reliable} (and part of cycle)
    ...
    \begin{proof*}
      ...
      Then define $T^{\prime\prime} = T^\prime \cup \{e^\prime\}$. Note $T^{\prime\prime}$ is
      connected as $e^\prime$ created a path between the two connected components of $T^\prime$.
      Additionally, $T^{\prime\prime}$ has the exactly number of edges as $T$, so $T^{\prime\prime}$
      is a valid spanning tree of $G$. But its weight:
      \[
      ...
      Hence if $e \in T$ then $T$ cannot be the minimum weight spanning tree for $G$.
    \end{proof*}
